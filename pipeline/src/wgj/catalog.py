...
belongs in `wgj manifest`, which runs when the data changes rather
...
from wgj.levels import LABELS as LEVELS
from wgj.paths import SCHEMAS, data_base, data_dir
from wgj.paths import rel as rel_path
from wgj.text import group_slug as slug
from wgj.text import human_bytes
...
def render_dataset(out: list[str], manifest: dict, ds: dict, up: str, cdn: str, raw: str) -> None:
    ...
    if ds.get("_preview_asset"):
        ...
        out.append(
            ...
            "    Generate one with `wgj previews` — see\n"
...
def generate(docs_dir: Path, cdn: str, raw: str) -> int:
    ...
    for manifest_path in sorted(data_dir().glob("*/*/manifest.json"), key=lambda p: p.as_posix()):
        ...
        _EDIT_TARGETS[(Path("catalog") / rel).as_posix()] = rel_path(manifest_path)
