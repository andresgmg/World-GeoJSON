"""Finalize committed GeoJSON: stable ids, hierarchy, shapeISO fixes, bbox.
    ...
    wgj finalize data/earth/CHL
    wgj finalize data/earth/*/
    wgj finalize --check data/earth/*/   # CI: exit 1 if stale
...
- `id`            Feature-level, `{ISO3}:{LEVEL}:{key}`. The key is the real,
                  ...
                  it). Manual keys live in wgj/tables/id_overrides.json.
- `shapeISO`      Upstream errors corrected from wgj/tables/shapeiso_fixes.json;
...
Previews are NOT touched here: `wgj previews` regenerates them from the
...
from wgj.levels import LEVELS
from wgj.paths import rel
from wgj.registry import countries, id_overrides, shapeiso_fixes
from wgj.text import slug
...
def assign_keys(
    ...
    if dupes:
        ...
        raise SystemExit(
            ...
            f"Resolve in wgj/tables/id_overrides.json."
        )
...
class Country:
    def __init__(self, d: Path) -> None:
        self.dir = d
        self.iso3 = d.name
        self.fixes = shapeiso_fixes().get(self.iso3, {})
        self.overrides = id_overrides().get(self.iso3, {})
        ...
        def hierarchy(p: dict) -> tuple[str | None, str | None, str | None]:
            ...
            if parent_key not in keys_by_level[parent_level]:
                raise SystemExit(
                    ...
                    f"Re-split the level (wgj build --resplit {self.iso3})."
                )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check and stale_total:
        print(f"\n{stale_total} file(s) need `wgj finalize`")
        return 1
    return 0
