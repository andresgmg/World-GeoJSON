...
End-to-end runbook. Budget an hour for your first one. The `wgj` command does
most of the work; the order matters — previews before the manifest, the index
after it.
...
the source turns out to be unusable. `wgj fetch --dry-run` (step 3)
...
pip install -r requirements-docs.txt -r requirements-dev.txt   # mkdocs; the wgj command
npm ci                                               # mapshaper, pinned
...
Add an entry to `pipeline/src/wgj/tables/countries.json`. Using New Zealand
and geoBoundaries as the worked example:
...
`parentID`, a `shapeISO` corrected from
`pipeline/src/wgj/tables/shapeiso_fixes.json` (or `""` where the upstream
has no code) and a recomputed `bbox`, in the
...
!!! note "If the build stops on a duplicate `id`"
    ...
    `wgj finalize` refuses and names them. Fix the cause: an entry in
    `pipeline/src/wgj/tables/shapeiso_fixes.json` if the upstream code is
    wrong, or a pinned key in `pipeline/src/wgj/tables/id_overrides.json`
    otherwise. See
...
Before the manifest, not after: `wgj manifest` records a preview only if it
already exists. See [Simplification & previews](previews.md) for the size
budget.
...
Zero errors before you open the PR — these are the checks CI runs, alongside
a `wgj manifest` regeneration that must leave no diff. Warnings — a dataset
without a preview, coordinates beyond 6 decimals — do not fail CI but do get
review comments.

Steps 4 to 8 are also one command: `wgj all NZL` runs build, previews,
manifest, index and validate in that order and stops at the first failure.
Use it for a re-run once the registry entry and the fetched sources are in
place.
...
Re-run steps 3 to 8 for that country. `wgj build` leaves the manifest's
hand-authored fields alone; `wgj manifest` rewrites `datasets` and carries
each entry's `license` forward.

## A source the pipeline does not know
...
`data/earth/XXX/`, run `wgj finalize data/earth/XXX` to give them their ids,
hierarchy and canonical layout, and run steps 5 to 8. Then add `license` to
every entry in `datasets` by hand — `wgj validate` rejects a dataset without
one, and only `wgj build` writes it — plus the identity block shown in step 4.
Teaching `wgj build` the new provider — a module under `wgj.sources` — is the
better contribution if you expect to repeat it.
